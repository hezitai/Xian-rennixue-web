...
YX.fn.showMe = function () {
    ...
    this.$userPic.attr('src', getAvatar("http://www.classbro.ca/" + user.avatar));
...
YX.fn.openChatBox = function (account, scene) {
    ...
    if (scene == 'p2p') {
        ...
        if (info) {
            ...
            var getOrderNo = JSON.parse(info.serverCustom);
            if(getOrderNo.orderNo){
                this.$nickName.text(getOrderNo.orderNo);
            } else{
    ...
    switch (type) {
        case 'session':
            ...
            if (scene === 'p2p') {
                ...
                if (teamInfo) {
                    var teamNickName = JSON.parse(teamInfo.serverCustom).orderNo;
                    if(!teamNickName){
                        info.nick = teamInfo.name;
                    } else {
                        info.nick = teamNickName;
                    }
                    if (teamInfo.avatar) {
                        info.avatar =
        ...
        case 'friend':
            ...
            break;
        case 'team':
            var getDataParse = JSON.parse(data.serverCustom);
            if(getDataParse.orderNo){
                info.nick = getDataParse.orderNo
            } else {
                info.nick = data.name;
            };
            info.type = data.type;
